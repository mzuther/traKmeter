...
(\SI{1.23}{\VRMS}) and leaves a headroom for peaks of about
\SI{20}{\dB}.  This in turn is consistent with the maximum crest
factor of analog audio signals.
...
Another recommendation (which many radio stations have been following
for reasons outside the scope of this document) is that peak levels
should not exceed \textbf{\SI{-9}{\dBFS}}.  For recording, this will
leave enough space for sudden jumps in level and also for inter-sample
peaks -- audio peaks that lie \emph{in between} two successive samples
and may lead to unpredictable clipping during digital-to-analog
conversion.
...
meter scales by \SI[retain-explicit-plus]{+20}{\dB}.  Thus, the
optimal average audio level is designated \textbf{\SI{0}{\dB} RMS},
while the maximum peak audio level becomes
\textbf{\SI[retain-explicit-plus]{+11}{\dB}}.  As a nice side effect,
our new scale corresponds to Bob Katz's
...
aligned to ``\SI[retain-explicit-plus]{+4}{\dBu}
