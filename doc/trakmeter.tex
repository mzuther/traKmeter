...
\item[Use high bit depths.]  Do yourself a favour and record at a bit
  ...
  amount of extra detail.  When disk space runs low, choose more
  (true) bits over a higher sampling rate.  See the
...
\item[Use sensible sampling rates.]  According to a
  ...
  rates for recording audio.  Support for \SI{88.1}{\kilo\hertz} is
  limited, so I prefer to work at \SI{96}{\kilo\hertz}.
...
\item[Fix it now.]  Contrary to popular belief, you cannot ``fix it in
  ...
  a last resort.  Apart from the time you lose with editing, it's easy
  to kill all of a track's vibe in the process.
...
\item[Make it exciting.]  A lot of today's music sounds like (and
  ...
  inaccuracies of human players and often makes these songs boring and
  lifeless.
