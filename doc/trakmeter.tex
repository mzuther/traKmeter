...
signal, you can try the other settings.  They shift the meter scales
down by \SI{5}{dB} to \SI{10}{dB} and add additional headroom at the
cost of a higher noise floor.  However, the signal-to-noise ratio of
modern converters is so great that you probably won't even notice.
...
can be adjusted using the ``Recording level'' buttons (see
\ref{sec:headroom_buttons}).
...
same on both peak and average meters.  Similar to a VU meter, the
...
above.  It has a rise time of one sample and fades out when the level
...
\item[Use high bit depths.]  Do yourself a favour and record at bit
  ...
  \href{http://www.aes.org/par/d/\#data_converter_bits}{AES}
...
\item[Try different sample rates.]  Recording at certain sample rates
  ...
  \href{http://www.lavryengineering.com/pdfs/lavry-white-paper-the_optimal_sample_rate_for_quality_audio.pdf}{Lavry
    Engineering}
