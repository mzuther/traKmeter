...
nominal level of \textbf{\SI[retain-explicit-plus]{+4}{\dBu}}
(\SI{1.23}{\VRMS}).  This leaves a headroom for peaks of at least
\SI{20}{\dB} to the clipping point.  Thus, driving all analogue audio
...
Now let's transfer this to the digital domain.  First, choose a
\href{https://www.digido.com/portfolio-item/level-practices-part-1/}{sane
  analogue reference level} for your converter (for me,
\SI[retain-explicit-plus]{+4}{\dBu} = \SI{0}{VU} = \SI{-15}{\dBFS}
works well).  Then, record with an average input level of
...
I also recommend that digital recording levels should not peak above
\textbf{\SI{-10}{\dBFS}}.  This will leave enough space for sudden
jumps in level and for inter-sample peaks -- audio peaks that lie
\emph{in-between} two successive samples and can lead to clipping
during digital-to-analogue conversion.

Keeping peak levels low may also improve the sound of your
analogue-to-digital converters.  Some converters degrade audio when
...
levels close to but not exceeding \SI{0}{\dBFS} (decibel relative to
digital full-scale).  Unfortunately, this misinformation has ended up
in many manuals for professional audio equipment.  But that doesn't
make it any truer.

Even if you let your recordings \emph{peak} at \SI{-20}{\dBFS} and you
...
bits\footnote{In the real world, all analogue circuits are
  ...
  more information.}  that you can capture with good converters \dots
...
You may have noticed that the \textbf{average} meter's green area is
centred around the \textbf{\SI{-20}{\dBFS}} mark.  This number should
be vaguely familiar.  Remember, this is the level we have determined
to be the optimal average recording level in the digital domain.

The highest yellow LED on the \textbf{peak} meter's top end
corresponds to a level of \textbf{\SI{-10}{\dBFS}}.  Again, this
number should be familiar: peak levels shouldn't exceed this level in
digital recordings.

Thus, by keeping the meter's readout \textbf{in the green} areas and
\textbf{out of the red} areas, you will automatically track at optimal
audio levels.  It's as simple as that!
...
should adjust the tracks to optimal loudness using traKmeter and a
(good!) gain plug-in.  Make sure that the gain treatment precedes all
other processing.

Mixing levels will now be much lower than what you are used to.  This
can easily be corrected by either adjusting the output gain of your
subgroups or by inserting a gain plug-in in your master track.

To preserve all transients, the final loudness of your mix should stay
within a certain average level ranges.  My plug-in
