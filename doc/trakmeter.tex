...
\SI{-9}{\dBFS} (\href{http://tech.ebu.ch/publications/r068}{EBU
  R68-2000}) during tracking.  We'll round that to
\textbf{\SI{-10}{\dBFS}} so that it's easier to remember.  In any
case, this will leave enough space for inter-sample peaks (audio peaks
that lie \emph{in between} two successive samples and may lead to
unpredictable clipping during digital-to-analog conversion).
...
peak audio level becomes \textbf{\SI[addsign=all]{+10}{\dB}}.  As a
nice side effect, our new scale corresponds to Bob Katz's
\href{http://www.digido.com/level-practices-part-2-includes-the-k-system.html}{K-20
  scale}.
...
scenario): even if your recordings \emph{peak} at \SI{-20}{\dBFS} and
you discard the least significant bit (some people claim that it
mostly consists of errors), this still leaves you with a
signal-to-noise ratio of \SI{70}{\dB}.  That is about what you can
expect from some of the best professional analog tape machines and
recording desks -- and we're not even talking of 24 bit.
...
A fully lit green LED on the peak meter's top end corresponds to a
level of \textbf{\SI[addsign=all]{+10}{\dB}} (or \SI{-10}{\dBFS}).
...
\textbf{\SI{0}{\dB} RMS} on the average meter.  Make sure that the
peak level never exceeds \textbf{\SI[addsign=all]{+10}{\dB}}.  In case
both conditions cannot be met simultaneously, adjust the peak level
only.
...
\ref{sec:controls_mixing_button}) and adjust volume using the gain
knob.
...
This button changes the meter scale.  When it is pressed, meter
readout uses the K-20 scale.  Click the button to change to decibels
...
and exhibits a completely flat frequency response.  It has been
...
The peak level meter possesses a completely flat frequency response.
