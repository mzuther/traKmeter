...
#include "three_meter.h"

ThreeMeter::ThreeMeter(const String& componentName, int posX, int posY, int width, int CrestFactor, int nNumChannels, int segment_height)
{
...
int ThreeMeter::getPreferredHeight()
{
    return nHeight;
}


void ThreeMeter::visibilityChanged()
{
...
void ThreeMeter::paint(Graphics& g)
{
    int x = 0;
    int y = 0;
...
void ThreeMeter::resized()
{
}


void ThreeMeter::setLevels(MeterBallistics* pMeterBallistics)
{
    for (int nChannel = 0; nChannel < nInputChannels; nChannel++)
    {
...
void ThreeMeter::drawMarkers(Graphics& g, String& strMarker, int x, int y, int width, int height, const Colour& colour)
{
    g.saveState();
